...
class ProductionOrderController extends Controller
{
    ...
    {
        return view('backend.productionorder.index');
    }

    public function printexportproductionorder($dateInput = false, $dateOutput = false, $personal = false, ?bool $grouped = false)
    {
        if($dateInput == 0){
            $dateInput = false;
        }
        ...
        $result = ProductionOrder::query()->with('personal', 'product_service_orders', 'service_type', 'order.user', 'order.departament')
        ...
        $pdf = PDF::loadView('backend.productionorder.print-export-production-order',compact('result', 'dateInput', 'dateOutput', 'getPersonal', 'grouped'))->setPaper('a4', 'portrait')
                  ->setWarnings(false);
    ...
    {
        return view('backend.productionorder.deleted');
    }
}
