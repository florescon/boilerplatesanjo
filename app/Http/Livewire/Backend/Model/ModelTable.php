...
class ModelTable extends TableComponent
{
    ...
    {
        $query = ModelProduct::query()->with('products');
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('Slug'), 'slug')
                ->searchable()
                ->sortable()
                ->format(function(ModelProduct $model) {
                    return $this->html($model->slug ?: '<span class="badge badge-secondary">'.__('undefined').'</span>');
                })
                ->exportFormat(function(ModelProduct $model) {
                    return $model->slug;
                }),
            Column::make('# '.__('Associated products'), 'count_products')
                ->format(function(ModelProduct $model) {
                    return $this->link(route('admin.model.associates', $model->id), $model->count_products);
                }),
            ...
            Column::make(__('Actions'))
                ->format(function (ModelProduct $model) {
                    return view('backend.model.datatable.actions', ['model' => $model]);
                })
                ->excludeFromExport(),
        ];
    }
