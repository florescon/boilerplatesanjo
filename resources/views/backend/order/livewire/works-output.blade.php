...
@php
    // Recolectar y ordenar todos los datos
    $allBatches = collect();
    ...
    foreach($order->getSizeTablesData($status->getStatusCollection()) as $parentId => $tableData) {
        if(is_int($parentId) && $order->getBatchForStatus($status->id, $parentId)) {
            $batches = $order->getBatchForStatus($status->id, $parentId);
            $allBatches = $allBatches->merge($batches);
        }
    }
    
    // Ordenar por created_at (más reciente primero)
    $sortedBatches = $allBatches->sortByDesc('created_at');
@endphp

@if($sortedBatches->isNotEmpty())
    <ul class="list-group list-group-flush mt-2">
        @foreach($sortedBatches as $batch)
            <a href="{{ route('admin.order.production_batch', [$order->id, $batch->id]) }}" 
               class="list-group-item d-flex justify-content-between align-items-center {{ $batch->allItemsAreInactiveAndBalanced() ? 'list-group-item-success' : 'list-group-item-danger' }} list-group-item-action">
                <div class="row w-100">
                    <div class="col-4">
                        Folio #{{ $batch->folio ?? $batch->id }}
                        <span class="badge badge-secondary badge-pill">{{ optional($batch->personal)->name }}</span>
                        <br>
                        <small class="text-muted">{{ $batch->created_at->format('d/m/Y H:i') }}</small>
                    </div>
                    <div class="col-4 text-right">
                        <span class="badge badge-secondary badge-pill">
                            <h5>Activo: <strong class="text-danger">{{ $order->getTotalBatchActiveProduction($status->id, $batch->parent_id ?? $batch->product_id, $batch->id) }}</strong></h5>
                        </span>
                    </div>
                    <div class="col-4 text-right">
                        <h5>
                            @if($batch->allItemsAreBalanced())
                                <span class="badge badge-success ml-4">Total recibido</span>
                            @endif
                            <span class="badge badge-primary badge-pill">{{ $order->getTotalBatchProduction($status->id, $batch->parent_id ?? $batch->product_id, $batch->id) }}</span>
                        </h5>
                    </div>
                </div>
            </a>
...
<div class="text-right col mb-3" wire:ignore>
    <button wire:click="saveOutput" wire:loading.attr="disabled" wire:target="saveOutput"  class="btn btn-sm btn-primary btn-hover">
        Dar salida
        <span wire:loading wire:target="saveOutput">
            ...
        </span>
    </button>
</div>
