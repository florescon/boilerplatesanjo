        ...
        <div class="row input-daterange mt-4">
            ...
            <div class="col-md-3">
                <div class="btn-group mr-2" role="group" aria-label="First group">
                    <button type="button" class="btn btn-outline-primary" wire:click="clearFilterDate"
                        class="btn btn-default">@lang('Clear date')</button>
                    <button type="button" class="btn btn-primary" wire:click="clearAll"
    ...
    <div class="card-body">
        ...
        <div class="row mb-4">
            ...
            <div class="col">
                <div class="input-group">
                    <input wire:model.debounce.350ms="searchTerm" class="form-control" type="text"
        ...
        @if ($selectPage)
            <x-utils.alert type="primary">
                @unless ($selectAll)
                    <span>Tienes seleccionado <strong>{{ $activities->count() }}</strong> activities, ¿quieres seleccionar
                        <strong>{{ $activities->total() }} </strong> actividades?</span>
            ...
            <div class="row mt-4">
                <div class="col">
                    <div class="table-responsive text-center">
                        <table
                            class="table table-sm align-items-center table-striped table-flush table-bordered table-hover">
                            <thead style="background-color:blue; color:white;">
                                <tr>
                                    ...
                                    <th scope="col">
                                        <a style="color:white;" wire:click.prevent="sortBy('log_name')" role="button"
                                            href="#">
                                            @lang('Name')
                                            @include('backend.includes._sort-icon', [
                                                'field' => 'log_name',
                                            ])
                                        </a>
                                    </th>
                                ...
                                @foreach ($activities as $activity)
                                    <tr>
                                        ...
                                        <th scope="row">
                                            <div>
                                                <button type="button"
                                                    class="btn {{ in_array($activity->log_name, $filters) ? 'btn-primary text-white' : 'btn-outline-primary' }} btn-sm"
                                                    wire:click="$emit('filterByLogName', {{ $activity->id }})">
                                                    {{ __($activity->log_name) }} </button>
                                            </div>
                                        </th>
                                        <td>
                        ...
                        @if ($activities->count())
                            <div class="row">
                                ...
                                <div class="col-sm-3 text-muted text-right">
                                    Mostrando {{ $activities->firstItem() }} - {{ $activities->lastItem() }} de
