        ...
        <aside class="position-fixed bg-light border-left" style="right: 0%; top: 19%; width: 18%; height: calc({{ $floatButton ? '73vh' : '10vh' }}); overflow-y: auto; box-shadow: -2px 0 10px rgba(0,0,0,0.1);">
          ...
          <div class="p-3">
              ...
              {{-- <button wire:click="$toggle('floatButton')" class="btn btn-outline-primary btn-sm btn-lg btn-block mb-3">
                ...
                <p class="text-center text-primary"> Orden {{ $batchOne->order_id }} </p>

                <a class="form-control text-center btn-primary" target="_blank"  href="{{ route('admin.batch.report', $batchOne->id) }}">
                  Reporte del Lote
                </a>
...
<div class="card-header d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3"
    ...
    <div class="d-flex flex-wrap gap-2 ">
        ...
        <div class="btn-group mb-2 mt-2 btn-sm" role="group" aria-label="Estados">
            <button
                wire:click="selectStatus('processed')"
                class="btn {{ $selectedStatus === 'processed' ? 'btn-primary' : 'btn-outline-primary' }} btn-sm">
                Producción
            </button>

            <button
                wire:click="selectStatus('received')"
                class="btn {{ $selectedStatus === 'received' ? 'btn-primary' : 'btn-outline-primary' }} btn-sm">
                Recibido
            </button>

            <button
                wire:click="selectStatus('delivered')"
                class="btn {{ $selectedStatus === 'delivered' ? 'btn-primary' : 'btn-outline-primary' }} btn-sm">
                Entregado
            </button>
        </div>
        ...
        <a href="#" class="btn btn-outline-info btn-sm ml-4 mt-2 mb-2">
            ...
            @foreach($parents as $parentId => $parentName)
              ...
              <div class="card-body">
                <h5 class="card-title font-weight-bolder">{!! $parentName !!}</h5>
                      ...
                      <thead class="">
                        <tr>
                          ...
                          @foreach($sizesByParent[$parentId] ?? [] as $sizeT)
                            <td style="border: 3px solid; border-style: dotted;" class="text-center">{{ $sizeT->name }}</td>
                          @endforeach
                          <td class="text-center table-dark">Total</td>
                        </tr>
                      </thead>
                      <tbody>
                                    ...
                                    <td class="text-center" style="border: 1px solid;">
                                        @if(isset($matrix[$op->operation_id][$parentId][$items->id][$sizeM->id]))
                                          <div class="d-inline-block position-relative">
                                            ...
                                            {{ $matrix[$op->operation_id][$parentId][$items->id][$sizeM->id]->qty }}
                                            
                                            <input
                                                type="number"
                                                @if($batchItem->{$selectedStatus} == $matrix[$op->operation_id][$parentId][$items->id][$sizeM->id]->qty)                                              
                                                disabled
                                                @endif
                                                style="width: 70px; color: blue;"
                                                placeholder="{{ $batchItem->{$selectedStatus} }}"
                                                {{-- placeholder="{{ $batchItem->status_qty }}" --}}
                                                ...
                                                "
                                                min="1"
                                                max="{{$matrix[$op->operation_id][$parentId][$items->id][$sizeM->id]->qty ?? 0 }}"

                                                {{-- name="qty[{{ $matrix[$op->operation_id][$items->id][$sizeM->id]->id }}]"> --}}
                                                wire:model.lazy="quantities.{{ $op->operation_id }}.{{ $batchItem->id }}"
                                                name="quantities[{{ $matrix[$op->operation_id][$parentId][$items->id][$sizeM->id]->id }}]">
                                          </div>
                                        @endif
                                    </td>
                                @endforeach
            ...
            @endforeach
                ...
                <div class="text-right col" wire:ignore>
                    ...
                    >
                        <span wire:loading.remove wire:target="messageAlert('save', '{{ $op->operation_id }}')">
                            <i class="fas fa-save"></i> Guardar captura
                            <h5 class="d-inline">
                                <span class="badge badge-light text-dark" id="total-{{ $op->operation_id }}">0</span>
                            </h5>
                        </span>
                        <span wire:loading wire:target="messageAlert('save', '{{ $op->operation_id }}')">
...
<script>
  ...
  document.addEventListener('livewire:load', function () {
      ...
      let batch = new ApexCharts(
        ...
        colors: ['#008FFB','#00E396','#FEB019', 'purple']
