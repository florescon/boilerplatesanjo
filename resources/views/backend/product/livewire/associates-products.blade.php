<x-backend.card>
  ...
  <x-slot name="header">
    @lang('Associated products') - {{ $attribute->name }}
  </x-slot>
          ...
          <div class="col-sm-12">
            <div class="row">
              <div class="col-6">
                <div class="c-callout c-callout-info"><small class="text-muted">@lang('Associated products')</small>
                  <div class="text-value-lg">{{ $attribute->count_products }}</div>
                </div>
              </div>

              <div class="col-6">
                <div class="c-callout c-callout-danger"><small class="text-muted">Porcentaje de asociados del total de productos</small>
                  <div class="text-value-lg">{{ round($attribute->total_percentage, 2) }}%</div>
                </div>
              </div>
            </div>
            ...
            @if($selectPage)
              <x-utils.alert type="primary">
                @unless($selectAll)
                <span>Tienes seleccionado <strong>{{ $associates->count() }}</strong> productos, ¿quieres seleccionar  <strong>{{ $associates->total() }} </strong> productos?</span>
                  <a href="#" wire:click="selectAll" class="alert-link">Seleccionar todo</a>
                @else
                  <span>Actualmente seleccionaste <strong>{{ $associates->total() }}</strong> productos.</span>
                @endif
            ...
            <div class="progress-group">
              <div class="progress-group-header align-items-end">
                ...
                <div class="mfs-auto font-weight-bold mfe-2">{{ $attribute->count_products }}</div>
                <div class="text-muted small">({{ round($attribute->total_percentage, 2) }}%)</div>
              </div>
              <div class="progress-group-bars">
                <div class="progress progress-xs">
                  <div class="progress-bar bg-primary" role="progressbar" style="width: {{ $attribute->total_percentage }}%" aria-valuenow="{{ $attribute->total_percentage }}" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
              </div>
            </div>
          </div>
                ...
                <table class="table table-responsive-sm table-hover table-outline mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div>{!! $product->full_name_link_main !!}</div>
